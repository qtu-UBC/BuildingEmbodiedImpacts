...
def calculate_embodied_impacts():
    """
    Calculate embodied environmental impacts of buildings based on their bill of materials.
    This function is used by the Streamlit app to process uploaded files and generate results.
    ...
    try:
        ...
        os.makedirs(config.LOG_OUTPUT_PATH, exist_ok=True)
        ...
        # Calculate impacts for each recipe
        for recipe_name, recipe_df in recipe_sheets.items():
            ...
            filepath = os.path.join(config.RESULTS_OUTPUT_PATH, filename)
        ...
        merged_results = merge_sheets(config.RESULTS_OUTPUT_PATH)
        merged_filepath = os.path.join(config.RESULTS_OUTPUT_PATH, f"merged_results_{timestamp}.csv")
        merged_results.to_csv(merged_filepath, index=False)
        
        return results
        
    except Exception as e:
        raise Exception(f"Error in calculate_embodied_impacts: {str(e)}")
